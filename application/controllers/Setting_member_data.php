...
class Setting_member_data extends CI_Controller {
	...
	public function coop_group(){
		...
		$this->db->select('*');
		$this->db->from('( SELECT coop_mem_group.*,
									...
									ROW_NUMBER() OVER (ORDER BY parent_group.mem_group_id,department_group.mem_group_id ASC) as row 
									FROM coop_mem_group 
									LEFT JOIN coop_mem_group as department_group ON coop_mem_group.mem_group_parent_id = department_group.id
									LEFT JOIN coop_mem_group as parent_group ON department_group.mem_group_parent_id = parent_group.id
								WHERE 
									coop_mem_group.mem_group_type = 3 ) a');
	...
	public function coop_member_save(){
		...
		$data_insert['mem_type_status']	= @$_POST["mem_type_status"];
