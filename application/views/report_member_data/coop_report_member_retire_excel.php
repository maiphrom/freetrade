...
for($m = $month_start; $m <= $month_end; $m++){
	$i=0;
		...
		if(@$row_check['member_id']=='' && @$_GET['report_date']=='' && @$_GET['month']!=''){
			...
			$objPHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(3.68);
			...
			$objPHPExcel->getActiveSheet()->getColumnDimension('J')->setWidth(18.71);
	...
	for($d = $day_start; $d <= $day_end; $d++){
		...
		$i+=1;
		$objPHPExcel->getActiveSheet()->mergeCells('A'.$i.':J'.$i);
		$objPHPExcel->getActiveSheet()->SetCellValue('A'.$i, "วันที่ ".$d." เดือน ".$month_arr[$m]." ปี ".($year+543)." สมาชิกลาออกจากสหกรณ์  จำนวน   ".$check_num."   ราย ดังนี้" ) ;
		$objPHPExcel->getActiveSheet()->getStyle('A'.$i)->applyFromArray($titleStyle);
		$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':J'.$i)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		...
		if(!empty($rs)){
			foreach($rs as $key => $row){
				...
				if(!empty($rs_share)){
					foreach($rs_share as $key => $row_loan){
				...
				$objPHPExcel->getActiveSheet()->getStyle('D'.$i.':F'.$i)->applyFromArray($textStyleArray);
				$objPHPExcel->getActiveSheet()->getStyle('D'.$i.':F'.$i)->applyFromArray($borderBottom);
