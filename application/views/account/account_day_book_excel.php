...
$styles_body = array( $styles_body_center, $styles_body_left, $styles_body_center, $styles_body_right, $styles_body_right);

$header = array(''=>'string',''=>'string',''=>'string',''=>'string',''=>'string');
$sheet = 'sheet1';
	$count_row=0;
	$writer->writeSheetHeader($sheet, $header, $col_options = ['widths'=>[10.30,45,10.29,12.43,12.43]] );
	...
	$count_row++;
	$writer->markMergedCell($sheet, $start_row=$count_row, $start_col=0, $end_row=$count_row, $end_col=4);
	$count_row++;
	$writer->markMergedCell($sheet, $start_row=$count_row, $start_col=0, $end_row=$count_row, $end_col=4);
	...
	$writer->writeSheetRow($sheet, array('วันที่','รายการ','เลขที่','เดบิต','เครดิต'), $styles_header );
	$writer->writeSheetRow($sheet, array('','','บัญชี','',''), $styles_header2 );
	
		//echo"<pre>";print_r($data);exit;
		foreach($data as $key => $row){
			foreach($row['detail'] as $key2 => $row_detail){
				if(@$date_now == date('Y-m-d',strtotime($row['account_datetime']))){
					$date_show = '';
				}else{
					$date_show = date('Y-m-d',strtotime($row['account_datetime']));
				}
				$date_now = date('Y-m-d',strtotime($row['account_datetime']));
				
				$writer->writeSheetRow($sheet, array(
					$date_show!=''?$this->center_function->ConvertToThaiDate($date_show,'1','0'):'',
