							...
							<label class="g24-col-sm-4">
								<?php if(@$data_get['account_buy_id'] != ''){ 
									if(@$row['account_buy_status'] == '1'){ ?>
										<button type="button" onclick="cancel_account_buy('0','<?php echo $row['account_buy_id']; ?>')" class="btn btn-warning min-width-100">
											<span class="icon icon-close"></span>
											ยกเลิกการยกเลิกรายการซื้อ					
										</button>
									<?php }else if(@$row['account_buy_status'] == '0'){ ?>
										<button type="button" onclick="cancel_account_buy('1','<?php echo $row['account_buy_id']; ?>')" class="btn btn-danger min-width-100">
											<span class="icon icon-close"></span>
											ยกเลิกรายการซื้อ					
										</button>
									<?php }else{ ?>
			...
			<div id="hidden_space">
				...
				<div class="bs-example" data-example-id="striped-table">
					<table class="table table-bordered table-striped table-center">	
						...
						<?php if(@$data_get['account_buy_id'] != ''){ ?>
						<tr class="bg-primary table_footer" style="background-color:#0288d1;">
							<td align='right' colspan='2'>ยอดรวมสุทธิ</td>
							<td align='right'> <?php echo number_format($row['total_amount'],2); ?> บาท</td>
							<td></td>
						</tr>
						<?php }else{ ?>
						<tr class="bg-primary table_footer" style="background-color:#0288d1;display:none;">
							<td align='right' colspan='2'>ยอดรวมสุทธิ</td>
...
<?php
$link = array(
    'src' => 'ci_project/assets/js/coop_buy.js',
